...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        safe_z = cam_points[:, 2].clamp(min=1e-6)
        ...
        z2 = safe_z * safe_z
        J_proj[:, 0, 0] = fx / safe_z
        J_proj[:, 0, 2] = -fx * x / z2
        J_proj[:, 1, 1] = fy / safe_z
        J_proj[:, 1, 2] = -fy * y / z2
    ...
    def compute_gaussian_values(
        ...
        det = torch.linalg.det(covs2D).clamp(min=1e-8)
        cov_inv = torch.linalg.inv(covs2D)
        exponent = -0.5 * torch.einsum('nhwi,nij,nhwj->nhw', dx, cov_inv, dx)
        normalizer = 1.0 / (2.0 * np.pi * torch.sqrt(det))
        gaussian = normalizer.view(N, 1, 1) * torch.exp(exponent.clamp(max=0.0))
