...
class GaussianModel(nn.Module):
    ...
    def _init_scales(self, points3D_xyz: torch.Tensor) -> None:
        ...
        if K < 1:
            self.scales = nn.Parameter(torch.zeros((self.n_points, 3), dtype=torch.float32))
            return

        if knn_points is not None:
            points = points3D_xyz.unsqueeze(0)  # Add batch dimension
            dists, _, _ = knn_points(points, points, K=K + 1)
            knn_dists = torch.sqrt(dists[0, :, 1:])
        else:
            all_dists = torch.cdist(points3D_xyz, points3D_xyz)
            all_dists.fill_diagonal_(float("inf"))
            knn_dists = torch.topk(all_dists, k=K, largest=False).values
