...
def create_mask_from_points(points, img_h, img_w):
    ...
    polygon = [tuple(point) for point in points.tolist()]
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_laplacian = F.conv2d(
        foreground_img, kernel, padding=1, groups=foreground_img.shape[1]
    )
    blended_laplacian = F.conv2d(
        blended_img, kernel, padding=1, groups=blended_img.shape[1]
    )

    foreground_region = foreground_mask.bool().expand_as(foreground_laplacian)
    background_region = background_mask.bool().expand_as(blended_laplacian)
    foreground_values = foreground_laplacian[foreground_region]
    blended_values = blended_laplacian[background_region]

    num_values = min(foreground_values.numel(), blended_values.numel())
    if num_values == 0:
        return blended_img.sum() * 0.0

    # For a translated polygon, the flattened masked pixels keep the same scanline order.
    loss = F.mse_loss(blended_values[:num_values], foreground_values[:num_values])
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state, iter_num=5000):
    ...
    foreground_image_original = foreground_image_original.convert("RGB")
    background_image_original = background_image_original.convert("RGB")
